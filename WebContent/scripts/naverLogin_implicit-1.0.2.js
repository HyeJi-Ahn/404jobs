...
var naver_id_login = function (client_id, redirect_uri, re_url) {
     ...
     */
    this.login = function () {
        //		var naver_id_login = document.getElementById(elementId);
        ...
        if (this.popup) {
            ...
            location.href = naver_id_login_url;
        }
    ...
    this.parseCallBack = function () {
        ...
        var sp_queryString = queryString.split("#");
        queryString = sp_queryString[1];
        ...
        var p = this.getParams(sp_queryString[0]);
        
        if (p['re_url'] !== undefined) {
            this.re_url = p['re_url'];
        }
    ...
    this.getParams = function (str) {
        var p = [],
            hash;
        var hashes = (String(str).toLowerCase()).slice(str.indexOf('?') + 1).split('&');
    ...
    this.get_naver_userprofile = function (callback_func1) {
        $.ajax({
            ...
            success: function (result) {
                inner_profileParams.age = (result.response !== undefined) ? result.response.age : "";
                inner_profileParams.birthday = (result.response !== undefined) ? result.response.birthday : "";
                inner_profileParams.email = (result.response !== undefined) ? result.response.email : "";
                inner_profileParams.enc_id = (result.response !== undefined) ? result.response.enc_id : "";
                inner_profileParams.gender = (result.response !== undefined) ? result.response.gender : "";
                inner_profileParams.id = (result.response !== undefined) ? result.response.id : "";
                inner_profileParams.name = (result.response !== undefined) ? result.response.name : "";
                inner_profileParams.nickname = (result.response !== undefined) ? result.response.nickname : "";
                inner_profileParams.profile_image = (result.response !== undefined) ? result.response.profile_image : "";
                eval(callback_func1);
            },
