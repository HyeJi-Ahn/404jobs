(function () {
    ...
    Social.naverJoin = function (data, isIframe) {
        ...
        var qstring = Social.getPcDomain() + "/Join/M_NV_Regist?Id=" + data.Id + "&Acstkn=" + encodeURIComponent(data.Acstkn) + "&Re_Url=" + reurl;
    ...
    Social.googleStart = function () {
        var params = {
            'clientid': '751392317978-jjc5ov9kf0tgnr4anlbe2crbfjk3it1c.apps.googleusercontent.com',
            'cookiepolicy': 'single_host_origin',
            'requestvisibleactions': 'http://schemas.google.com/AddActivity',
            'scope': 'profile email',
            'callback': Social.attachSignin
        }
    ...
    Social.attachSignin = function (authResult) {
        var obj = this;
        if (authResult) {
            if (authResult['error'] == undefined && (authResult["status"]["method"] == "PROMPT" || authResult["status"]["method"] == "AUTO")) {
                ...
                gapi.client.load('plus', 'v1', function () {
                    ...
                    request.execute(function (res) {
                        var id = (res.id !== undefined) ? res.id : "";
                        var name = (res.displayName !== undefined) ? res.displayName : "";
                        var gender = (res.gender !== undefined) ? res.gender : "";
                        var email = "";
